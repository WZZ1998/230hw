...
ti env (EFst e)       = do (s, t) <- ti env e
                           tv1    <- freshTVbl "a"
                           tv2    <- freshTVbl "b"
                           s2     <- mgu (t) (tv1 `TCom` tv2)
                           return (s2 `after` s, apply s2 tv1)

ti env (ESnd e)       = do (s, t) <- ti env e
                           tv1    <- freshTVbl "a"
                           tv2    <- freshTVbl "b"
                           s2     <- mgu t (tv1 `TCom` tv2)
...
ti env (e1 `ECons` e2) = do (s1, t1) <- ti env e1
                            ...
                            return (((after (after s3 s2)) s1), apply s3 t2)

ti env (EIsNil e)      = do (s, t) <- ti env e
                            tv     <- freshTVbl "a"
                            s'     <- mgu t (TList tv)
                            return (after s' s, TBool)

ti env (EDcons e)      = do (s, t) <- ti env e
                            tv     <- freshTVbl "a"
                            s'     <- mgu t (TList tv)
                            return (after s' s, TCom (apply s' tv) (apply s' t))
...
--                             let s3   = after s2 s1
--                                 t2   = generalize (apply s3 env) (apply s3 t1)
--                                 env2 = env
--                             (s4, t3) <- ti (apply s3 env2) e2
--                             return (after s4 s3, t3)
...
prType (TCom t1 t2) = PP.parens (prType t1 PP.<+> PP.char ',' PP.<+> prType t2)
...
prExp (ECons e1 e2)    = PP.parens (prExp e1 PP.<+> PP.colon PP.<+> prParenExp e2)
