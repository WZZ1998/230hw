...
\begin{code}
{-@ length :: xs:List a -> {v:Int | size xs = v} @-}
length            :: List a -> Int
length Emp        = 0
...
replicate n x
  | n<0       = error "length can't be negative"
  | n==0      = empty
...
{-@ concat :: xss:List (List a) -> {xs:List a| (size xs) = sizes xss} @-}
concat Emp  = Emp
concat ((x:+:xs):+:xss) = x:+:(concat (xs:+:xss))
concat ((x:+:empty) :+: xss) = x:+:(concat (xss))
concat (empty :+: xss)       = concat xss
